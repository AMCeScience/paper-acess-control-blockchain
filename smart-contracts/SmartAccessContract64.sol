...
contract Policies {
    ...
    function createPolicy(string memory _context_expression, uint8 _actions, uint256 _resource_id, uint64 _policy, bool _conventional_access, bool _emergency_access)public{
    ...
    function changePolicy(uint256 _uuid, uint64 _policy) public {
        require(msg.sender == owner);
        if (getPolicy(_uuid)) {
...
contract Decision {
    ...
    event invalidHashComparison(uint64 _attributes, string _public_key, bytes32 _hash);
    event validHashComparison(uint64 _attributes, string _public_key, bytes32 _hash);
    event attrsComplyingWithPolicy(Policies.Policy _policy, uint64 _attributes);
    event attrsNotComplyingWithPolicy(Policies.Policy _policy, uint64 _attributes);
    ...
    function evaluateRequest(string memory _context_expression, uint256 _resource_id, uint8 _actions, bytes32 _hashed_token, bytes memory _signed_token, uint64 _attributes, address _processor_addr) public {        
    ...
    function signatureAndProfessionalVerification(bytes32 _hashed_token, uint64 _attr, string memory _public_key) public returns (bool) {
    ...
    function verifyPolicy(string memory _context_expression, string memory _public_key, uint256 _resource_id, uint8 _actions, uint64 _attributes) public {
    ...
    function policyCompliant(Policies.Policy memory loaded_policy, uint64 _attrs) public returns(bool){
        if ((bytes8(loaded_policy.policy) & bytes8(_attrs)) == bytes8(loaded_policy.policy)){
...
contract DataAccess {
    ...
    function requestAccess(string memory _context_expression, uint256 _resource_id, uint8 _actions, bytes32 _hashed_token, bytes memory _signed_token, uint64 _attributes) public{
