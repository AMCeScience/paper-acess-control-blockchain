...
contract Policies {
    ...
    function createPolicy(string memory _context_expression, uint8 _actions, uint256 _resource_id, uint256 _policy, bool _conventional_access, bool _emergency_access)public{
    ...
    function changePolicy(uint256 _uuid, uint256 _policy) public {
        require(msg.sender == owner);
        if (getPolicy(_uuid)) {
...
contract Decision {
    ...
    event invalidHashComparison(uint256 _attributes, string _public_key, bytes32 _hash);
    event validHashComparison(uint256 _attributes, string _public_key, bytes32 _hash);
    event attrsComplyingWithPolicy(Policies.Policy _policy, uint256 _attributes);
    event attrsNotComplyingWithPolicy(Policies.Policy _policy, uint256 _attributes);
    ...
    function evaluateRequest(string memory _context_expression, uint256 _resource_id, uint8 _actions, bytes32 _hashed_token, bytes memory _signed_token, uint256 _attributes, address _processor_addr) public {        
    ...
    function signatureAndProfessionalVerification(bytes32 _hashed_token, uint256 _attr, string memory _public_key) public returns (bool) {
    ...
    function verifyPolicy(string memory _context_expression, string memory _public_key, uint256 _resource_id, uint8 _actions, uint256 _attributes) public {
    ...
    function policyCompliant(Policies.Policy memory loaded_policy, uint256 _attrs) public returns(bool){
        if ((bytes32(loaded_policy.policy) & bytes32(_attrs)) == bytes32(loaded_policy.policy)){
...
contract DataAccess {
    ...
    function requestAccess(string memory _context_expression, uint256 _resource_id, uint8 _actions, bytes32 _hashed_token, bytes memory _signed_token, uint256 _attributes) public{
